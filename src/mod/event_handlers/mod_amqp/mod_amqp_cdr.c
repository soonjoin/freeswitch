...
*
* mod_amqp.c -- Sends FreeSWITCH cdr to an AMQP broker
*
*/

#include "mod_amqp.h"

static switch_status_t mod_amqp_cdr_reporting(switch_core_session_t *session)
{
	...
	cJSON *cdr_json = NULL;
	char *cdr_json_text = NULL;

	// generate_json_cdr
	if (switch_ivr_generate_json_cdr(session, &cdr_json, SWITCH_TRUE) == SWITCH_STATUS_SUCCESS) {
		/* build the json */
		cdr_json_text = cJSON_Print(cdr_json);
		cJSON_Delete(cdr_json);
	}
	
	if (cdr_json_text == NULL) {
		return SWITCH_STATUS_FALSE;
	}

    // Loop through cdr hash of profiles.
    for (hi = switch_core_hash_first(mod_amqp_globals.cdr_hash); hi; hi = switch_core_hash_next(&hi)) {
		...
		if (!profile) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Session without a profile %p %p\n", (void *)session, (void *)profile);
			continue;
		}

		/* If the mod is disabled ignore the state report*/
		if (!profile->running) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Profile[%s] not running\n", profile->name);
			continue;
		}

		/* If the circuit breaker is active, ignore the state report*/
		...
		amqp_message->pjson = strdup(cdr_json_text);
		mod_amqp_cdr_routing_key(profile, amqp_message->routing_key, session, profile->format_fields);
	...
	switch_safe_free(cdr_json_text);
											  ...
											  switch_core_session_t *session, mod_amqp_keypart_t routingKeyChannelVariableNames[])
{
	...
	for (i = 0; i < MAX_ROUTING_KEY_FORMAT_FIELDS && idx < MAX_AMQP_ROUTING_KEY_LENGTH; i++) {
		if (routingKeyChannelVariableNames[i].size) {
			if (idx) {
				routingKey[idx++] = '.';
			}
			for( x = 0; x < routingKeyChannelVariableNames[i].size; x++) {
...
switch_status_t mod_amqp_cdr_create(char *name, switch_xml_t cfg)
{
	...
	profile->exchange = exchange ? exchange : switch_core_strdup(profile->pool, "TAP.Cdr");
	...
	profile->conn_active = NULL;
	/* We are not going to open the cdr queue connection on create, but instead wait for the running thread to open it */
	...
	/* Start the cdr send thread. This will set up the initial connection */
	switch_threadattr_create(&thd_attr, profile->pool);
	switch_threadattr_stacksize_set(thd_attr, SWITCH_THREAD_STACKSIZE);
	if (switch_thread_create(&profile->cdr_thread, thd_attr, mod_amqp_cdr_thread, profile, profile->pool)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Cannot create 'amqp cdr sender' thread!\n");
		goto err;
	}
...
switch_status_t mod_amqp_cdr_send(mod_amqp_cdr_profile_t *profile, mod_amqp_message_t *msg)
{
	...
	if (status < 0) {
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "Profile[%s] failed to send cdr on connection[%s]: %s\n",
...
void * SWITCH_THREAD_FUNC mod_amqp_cdr_thread(switch_thread_t *thread, void *data)
{
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Cdr sender thread stopped\n");
